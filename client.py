...
with st.sidebar:
    ...
    mode = st.radio(
        "Select Mode",
        ["LLM", "Web Search", "Hybrid"],
        help="LLM : Uses the language model\n"
             "Web Search : Searches the web and summarizes\n"
             "Hybrid: Intelligently combines web search with LLM when needed"
    )
    
    mode_descriptions = {
        "LLM": "Quick responses using the model's knowledge",
        "Web Search": "Web search results with sources",
    ...
    if st.button("Clear Chat History", help="Clear all chat history"):
...
def extract_sections(text):
    ...
    reason_match = re.search(r'Reason for web augmentation:(.*?)(?=\n\n🌐|\n\n📚|$)', text, re.DOTALL)
    if reason_match:
        ...
        if "reasoning_data" in msg and chat_mode == "Hybrid":
            ...
            if reasoning_data["llm_response"]["complete"]:
                ...
                <div class="chain-of-thought" style="margin-top: 15px;">
                    <div class="chain-header">Reasoning</div>
                """
