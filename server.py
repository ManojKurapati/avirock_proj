...
async def chat_hybrid(req: Request):
    ...
    try:
        ...
        # 5. Summarize web results
        web_summary = await summarize_snippets(snippets)

        # 6. Create web-informed prompt and generate augmented response
        ...
        # 7. BONUS: Generate follow-up question
        followup_prompt = (
            f"Based on the above web context and the user's query: '{query}', "
            f"suggest a single follow-up question that would help clarify or expand the user's understanding."
        )
        followup_question = ""
        async for token in llm.astream(followup_prompt):
            followup_question += token

        # 8. Stream final output
...
async def openai_compatible_chat(req: Request):
    try:
        ...
        # Optional: prepend full chat history
        full_prompt = "User: " + user_prompt + "\nAssistant:"

        if stream:
            return StreamingResponse(
                stream_llm(full_prompt),
                media_type="text/event-stream"
            )
        else:
            final = ""
            async for chunk in stream_llm(full_prompt):
                final += chunk
            return {
                "id": "chatcmpl-openai-style",
        ...
        raise HTTPException(500, f"OpenAI-style error: {str(e)}")
